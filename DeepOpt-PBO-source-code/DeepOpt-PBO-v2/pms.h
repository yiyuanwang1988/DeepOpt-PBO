...
void Satlike::allocate_memory()
{
	...
	hhscore = new long long[malloc_var_length];
	...
	best_soln_vo = new int[malloc_var_length];
	...
	score_baocun = new long long[10]; // fps
	sscore_baocun = new long long[10];
	hhscore_baocun = new long long[10];
	score2 = new long long[malloc_var_length];
	sscore2 = new long long[malloc_var_length];
	hhscore2 = new long long[malloc_var_length];
...
void Satlike::init_vt(vector<int> &init_solution)
{
	int v, c;
	int j;
	...
	else if (tries > 1)
	{
		...
		for (v = 1; v <= num_vars; v++)
		{
			...
			else if (rand() % 100 < 1 && time_stamp[v] == 0)
			{
				cur_soln[v] = 1 - cur_soln[v];
				time_stamp[v]++;
			}
			else if (time_stamp[v] != 0)
				time_stamp[v]--;
			unsat_app_count[v] = 0;
		}
		if (mark_tries % LBC == LBC - 1)
...
int Satlike::pick_var()
{
	...
	if (goodvar_stack_fill_pointer > 0)
	{
		...
		{
			int index = goodvar_stack[rand() % goodvar_stack_fill_pointer];
			time_stamp[index]++;
			return index;
		}
...
void Satlike::flip_fps(int flipvar)
{
	...
	int org_hhscore = hhscore[flipvar];
			...
			if (cur_soln[flipvar] == var_lit[flipvar][i].sense)
			{
				if (sat_count2[c] + weight <= clause_true_lit_thres[c])
				{
					...
					for (lit *p = clause_c; (v = p->var_num) != 0; p++)
					{
						...
						{
							score2[v] -= (unit_weight[c] * (min(gap, p->weight) - min(gap - weight, p->weight)));
							hhscore2[v] += (1 * (max(p->weight - gap + weight, 0) - max(p->weight - gap, 0)));
						}
					}
				}
				else if (sat_count2[c] <= clause_true_lit_thres[c]) // sat_count[c]+weight > clause_true_lit_thres[c]
				{
					gap = clause_true_lit_thres[c] - sat_count2[c];
					for (lit *p = clause_c; (v = p->var_num) != 0; p++)
					{
						if (p->sense != cur_soln[v])
						{
							score2[v] -= (unit_weight[c] * min(gap, p->weight));
							// hhscore[v] += (unit_weight[c] * (min(p->weight, gap1[c] - sat_count[c] - weight) - max(p->weight - gap, 0)));
							hhscore2[v] += (1 * (min(p->weight, gap1[c] - sat_count2[c] - weight) - max(p->weight - gap, 0)));
						}
						else
						{
							score2[v] += unit_weight[c] * (p->weight - max(0, gap - weight + p->weight));
							// hhscore[v] -= unit_weight[c] * min(p->weight, weight - gap);
							hhscore2[v] -= 1 * min(p->weight, weight - gap);
						}
					}
				}
					...
					if (sat_count2[c] <= gap1[c])
					{
						if (sat_count2[c] + weight <= gap1[c])
						{
							...
							for (lit *p = clause_c; (v = p->var_num) != 0; p++)
							{
								if (p->sense == cur_soln[v])
								{
									score2[v] += unit_weight[c] * (max(0, gap + p->weight) - max(0, gap - weight + p->weight));
									// hhscore[v] += unit_weight[c] * (min(p->weight, -gap) - min(p->weight, weight - gap));
									hhscore2[v] += 1 * (min(p->weight, -gap) - min(p->weight, weight - gap));
								}
								else
								{
									// hhscore[v] += unit_weight[c] * (min(p->weight, gap1[c] - sat_count[c] - weight) - min(p->weight, gap1[c] - sat_count[c]));
									hhscore2[v] += 1 * (min(p->weight, gap1[c] - sat_count2[c] - weight) - min(p->weight, gap1[c] - sat_count2[c]));
								}
							}
						}
						else if (sat_count2[c] + weight > gap1[c]) //(2)-(3)
						{
							...
							for (lit *p = clause_c; (v = p->var_num) != 0; p++)
							{
								...
								{
									score2[v] += unit_weight[c] * (max(0, gap + p->weight) - max(0, gap - weight + p->weight));
									// hhscore[v] += unit_weight[c] * (min(p->weight, -gap) - max(p->weight - weight - sat_count[c] + gap , 0));
									hhscore2[v] += 1 * (min(p->weight, -gap) - max(p->weight - weight - sat_count2[c] + gap, 0));
								}
								else
								{
									// hhscore[v] -= unit_weight[c] * (min(p->weight, gap1[c] - sat_count[c]));
									hhscore2[v] -= 1 * (min(p->weight, gap1[c] - sat_count2[c]));
								}
							}
						}
					}
					else if (sat_count2[c] > gap1[c])
					{
						gap = clause_true_lit_thres[c] - sat_count2[c];
						for (lit *p = clause_c; (v = p->var_num) != 0; p++)
						{
							...
							{
								score2[v] += unit_weight[c] * (max(0, gap + p->weight) - max(0, gap - weight + p->weight));
								// hhscore[v] += unit_weight[c] * (max(p->weight - sat_count[c] + gap1[c] , 0) - max(p->weight - sat_count[c] - weight + gap1[c], 0));
								hhscore2[v] += 1 * (max(p->weight - sat_count2[c] + gap1[c], 0) - max(p->weight - sat_count2[c] - weight + gap1[c], 0));
							}
						}
					}
				}
			...
			else // cur_soln[flipvar] != cur_lit.sense
			{
				if (sat_count2[c] - weight >= clause_true_lit_thres[c])
				{
					if (sat_count2[c] > gap1[c])
					{
						if (sat_count2[c] - weight > gap1[c])
						{
							...
							for (lit *p = clause_c; (v = p->var_num) != 0; p++)
							{
								if (p->sense == cur_soln[v])
								{
									score2[v] -= unit_weight[c] * (max(0, gap + weight + p->weight) - max(0, gap + p->weight));
									// hhscore[v] += unit_weight[c] * (max(p->weight - sat_count[c] + gap1[c], 0) - max(p->weight - sat_count[c] + weight + gap1[c], 0));
									hhscore2[v] += 1 * (max(p->weight - sat_count2[c] + gap1[c], 0) - max(p->weight - sat_count2[c] + weight + gap1[c], 0));
								}
							}
						}
						else if (sat_count2[c] - weight <= gap1[c])
						{
							gap = clause_true_lit_thres[c] - sat_count2[c];
							for (lit *p = clause_c; (v = p->var_num) != 0; p++)
							{
								...
								{
									score2[v] -= unit_weight[c] * (max(0, gap + weight + p->weight) - max(0, gap + p->weight));
									// hhscore[v] += unit_weight[c] * (max(p->weight - sat_count[c] + gap1[c], 0) - min(p->weight, sat_count[c] - weight - sat_count[c]));
									hhscore2[v] += 1 * (max(p->weight - sat_count2[c] + gap1[c], 0) - min(p->weight, sat_count2[c] - weight - sat_count2[c]));
								}
								else
								{
									// hhscore[v] += unit_weight[c] * (min(p->weight, gap1[c] - sat_count[c] + weight));
									hhscore2[v] += 1 * (min(p->weight, gap1[c] - sat_count2[c] + weight));
								}
							}
						}
					}
					else if (sat_count2[c] <= gap1[c])
					{
						...
						for (lit *p = clause_c; (v = p->var_num) != 0; p++)
						{
							...
							{
								score2[v] -= unit_weight[c] * (max(0, gap + weight + p->weight) - max(0, gap + p->weight));
								// hhscore[v] += unit_weight[c] * (min(p->weight, -gap) - min(p->weight, sat_count[c] - weight - clause_true_lit_thres[c]));
								hhscore2[v] += 1 * (min(p->weight, -gap) - min(p->weight, sat_count2[c] - weight - clause_true_lit_thres[c]));
							}
							else
							{
								// hhscore[v] += unit_weight[c] * (min(p->weight, gap1[c] - sat_count[c] + weight) - min(p->weight, gap1[c] - sat_count[c]));
				...
				else if (sat_count2[c] >= clause_true_lit_thres[c])
				{
					...
					for (lit *p = clause_c; (v = p->var_num) != 0; p++)
					{
						if (p->sense == cur_soln[v])
						{
							score2[v] -= unit_weight[c] * (p->weight - max(0, gap + p->weight));
							// hhscore[v] += unit_weight[c] * (p->weight, -gap);
							hhscore2[v] += 1 * (p->weight, -gap);
						}
						else
						{
							score2[v] += unit_weight[c] * min(p->weight, gap + weight);
							// hhscore[v] += unit_weight[c] * (min(p->weight - gap - weight, 0) - min(p->weight, gap1[c] - sat_count[c]));
							hhscore2[v] += 1 * (min(p->weight - gap - weight, 0) - min(p->weight, gap1[c] - sat_count2[c]));
						}
					}
				}
				else
				{
					...
					for (lit *p = clause_c; (v = p->var_num) != 0; p++)
					{
						if (p->sense != cur_soln[v])
						{
							score2[v] += unit_weight[c] * (min(p->weight, gap + weight) - min(p->weight, gap));
							// hhscore[v] += unit_weight[c] * (max(p->weight - gap - weight, 0) - max(p->weight - gap, 0));
							hhscore2[v] += 1 * (max(p->weight - gap - weight, 0) - max(p->weight - gap, 0));
						}
					}
				}
			...
			if (cur_soln[flipvar] == var_lit[flipvar][i].sense)
			{
				if (sat_count2[c] + weight <= clause_true_lit_thres[c])
				{
					...
					for (lit *p = clause_c; (v = p->var_num) != 0; p++)
					{
						if (p->sense != cur_soln[v])
						{
							sscore2[v] -= (unit_weight[c] * (min(gap, p->weight) - min(gap - weight, p->weight)));
						}
					}
				}
				else if (sat_count2[c] <= clause_true_lit_thres[c])
				{
					...
					for (lit *p = clause_c; (v = p->var_num) != 0; p++)
					{
						if (p->sense != cur_soln[v])
						{
							sscore2[v] -= (unit_weight[c] * min(gap, p->weight));
						}
						else
						{
							sscore2[v] += unit_weight[c] * (p->weight - max(0, gap - weight + p->weight));
						}
					}
				}
				else // sat_count[c]+weight > clause_true_lit_thres[c], sat_count[c] > clause_true_lit_thres[c]
				{
					...
					for (lit *p = clause_c; (v = p->var_num) != 0; p++)
					{
						if (p->sense == cur_soln[v])
						{
							sscore2[v] += unit_weight[c] * (max(0, gap + p->weight) - max(0, gap - weight + p->weight));
						}
					}
				}
				sat_count2[c] += weight;
			...
			else // cur_soln[flipvar] != cur_lit.sense
			{
				if (sat_count2[c] - weight >= clause_true_lit_thres[c])
				{
					...
					for (lit *p = clause_c; (v = p->var_num) != 0; p++)
					{
						if (p->sense == cur_soln[v])
						{
							sscore2[v] -= unit_weight[c] * (max(0, gap + weight + p->weight) - max(0, gap + p->weight));
						}
					}
				}
				else if (sat_count2[c] >= clause_true_lit_thres[c])
				{
					...
					for (lit *p = clause_c; (v = p->var_num) != 0; p++)
					{
						if (p->sense == cur_soln[v])
						{
							sscore2[v] -= unit_weight[c] * (p->weight - max(0, gap + p->weight));
						}
						else
						{
							sscore2[v] += unit_weight[c] * min(p->weight, gap + weight);
						}
					}
				}
				else
				{
					...
					for (lit *p = clause_c; (v = p->var_num) != 0; p++)
					{
						if (p->sense != cur_soln[v])
						{
							sscore2[v] += unit_weight[c] * (min(p->weight, gap + weight) - min(p->weight, gap));
						}
					}
				}
				sat_count2[c] -= weight;
...
void Satlike::update_goodvarstack12(int flipvar)
{
	...
	for (int i = 0; i < var_neighbor_count[flipvar]; ++i)
	{
		...
		if (score2[v] + sscore2[v] > 0) /*New Added*/
		{
				...
				if (sat_count[c] + weight <= clause_true_lit_thres[c])
				{
					...
					for (lit *p = clause_c; (v = p->var_num) != 0; p++)
					{
						if (p->sense != cur_soln[v])
						{
							...
							// hhscore[v] += (unit_weight[c] * (max(p->weight - gap + weight, 0) - max(p->weight - gap , 0)));
							hhscore[v] += (1 * (max(p->weight - gap + weight, 0) - max(p->weight - gap, 0)));
						}
					}
				}
				else if (sat_count[c] <= clause_true_lit_thres[c]) // sat_count[c]+weight > clause_true_lit_thres[c]
				{
					...
					for (lit *p = clause_c; (v = p->var_num) != 0; p++)
					{
						if (p->sense != cur_soln[v])
						{
							...
							// hhscore[v] += (unit_weight[c] * (min(p->weight, gap1[c] - sat_count[c] - weight) - max(p->weight - gap, 0)));
							hhscore[v] += (1 * (min(p->weight, gap1[c] - sat_count[c] - weight) - max(p->weight - gap, 0)));
						}
						else
						{
							score[v] += unit_weight[c] * (p->weight - max(0, gap - weight + p->weight));
							// hhscore[v] -= unit_weight[c] * min(p->weight, weight - gap);
							hhscore[v] -= 1 * min(p->weight, weight - gap);
						}
					}
				}
				else // sat_count[c]+weight > clause_true_lit_thres[c], sat_count[c] > clause_true_lit_thres[c]
						...
						if (sat_count[c] + weight <= gap1[c])
						{
							...
							for (lit *p = clause_c; (v = p->var_num) != 0; p++)
							{
								if (p->sense == cur_soln[v])
								{
									...
									// hhscore[v] += unit_weight[c] * (min(p->weight, -gap) - min(p->weight, weight - gap));
									hhscore[v] += 1 * (min(p->weight, -gap) - min(p->weight, weight - gap));
								}
								else
								{
									// hhscore[v] += unit_weight[c] * (min(p->weight, gap1[c] - sat_count[c] - weight) - min(p->weight, gap1[c] - sat_count[c]));
									hhscore[v] += 1 * (min(p->weight, gap1[c] - sat_count[c] - weight) - min(p->weight, gap1[c] - sat_count[c]));
								}
							}
						}
						else if (sat_count[c] + weight > gap1[c])
						{
							gap = clause_true_lit_thres[c] - sat_count[c];
							for (lit *p = clause_c; (v = p->var_num) != 0; p++)
							{
								if (p->sense == cur_soln[v])
								{
									score[v] += unit_weight[c] * (max(0, gap + p->weight) - max(0, gap - weight + p->weight));
									// hhscore[v] += unit_weight[c] * (min(p->weight, -gap) - max(p->weight - weight - sat_count[c] + gap , 0))
									hhscore[v] += 1 * (min(p->weight, -gap) - max(p->weight - weight - sat_count[c] + gap, 0));
								}
								else
								{
									// hhscore[v] -= unit_weight[c] * (min(p->weight, gap1[c] - sat_count[c]));
									hhscore[v] -= 1 * (min(p->weight, gap1[c] - sat_count[c]));
								}
							}
						}
					}
					else if (sat_count[c] > gap1[c])
					{
						...
						for (lit *p = clause_c; (v = p->var_num) != 0; p++)
						{
							if (p->sense == cur_soln[v])
							{
								score[v] += unit_weight[c] * (max(0, gap + p->weight) - max(0, gap - weight + p->weight));
								// hhscore[v] += unit_weight[c] * (max(p->weight - sat_count[c] + gap1[c] , 0) - max(p->weight - sat_count[c] - weight + gap1[c], 0));
								hhscore[v] += 1 * (max(p->weight - sat_count[c] + gap1[c], 0) - max(p->weight - sat_count[c] - weight + gap1[c], 0));
							}
						}
					}
				}
						...
						else if (sat_count[c] - weight <= gap1[c])
						{
							...
							for (lit *p = clause_c; (v = p->var_num) != 0; p++)
							{
								if (p->sense == cur_soln[v])
								{
									score[v] -= unit_weight[c] * (max(0, gap + weight + p->weight) - max(0, gap + p->weight));
									// hhscore[v] += unit_weight[c] * (max(p->weight - sat_count[c] + gap1[c], 0) - min(p->weight, sat_count[c] - weight - sat_count[c]));
									hhscore[v] += 1 * (max(p->weight - sat_count[c] + gap1[c], 0) - min(p->weight, sat_count[c] - weight - sat_count[c]));
								}
								else
								{
									// hhscore[v] += unit_weight[c] * (min(p->weight, gap1[c] - sat_count[c] + weight));
									hhscore[v] += 1 * (min(p->weight, gap1[c] - sat_count[c] + weight));
								}
							}
						}
					}
					else if (sat_count[c] <= gap1[c])
					{
						...
						for (lit *p = clause_c; (v = p->var_num) != 0; p++)
						{
							if (p->sense == cur_soln[v])
							{
								score[v] -= unit_weight[c] * (max(0, gap + weight + p->weight) - max(0, gap + p->weight));
								// hhscore[v] += unit_weight[c] * (min(p->weight, -gap) - min(p->weight, sat_count[c] - weight - clause_true_lit_thres[c]));
								hhscore[v] += 1 * (min(p->weight, -gap) - min(p->weight, sat_count[c] - weight - clause_true_lit_thres[c]));
							}
							else
							{
								// hhscore[v] += unit_weight[c] * (min(p->weight, gap1[c] - sat_count[c] + weight) - min(p->weight, gap1[c] - sat_count[c]));
				...
				else if (sat_count[c] >= clause_true_lit_thres[c])
				{
					...
					for (lit *p = clause_c; (v = p->var_num) != 0; p++)
					{
						if (p->sense == cur_soln[v])
						{
							...
							// hhscore[v] += unit_weight[c] * (p->weight, -gap);
							hhscore[v] += 1 * (p->weight, -gap);
						}
						else
						{
							score[v] += unit_weight[c] * min(p->weight, gap + weight);
							// hhscore[v] += unit_weight[c] * (min(p->weight - gap - weight, 0) - min(p->weight, gap1[c] - sat_count[c]));
							hhscore[v] += 1 * (min(p->weight - gap - weight, 0) - min(p->weight, gap1[c] - sat_count[c]));
						}
					}
				}
				else
...
void Satlike::print_best_solution(char *filename, char *seed)
{
	if (best_soln_feasible == 1)
	{
		...
		if (verify_sol())
			...
			cout << filename << " " << seed << " " << opt_unsat_weight << " " << obj_3600 << " " << opt_time << endl;
		else
...
void Satlike::local_search_vo(vector<int> &init_solution, char *inputfile)
{
	int step_count = 0;
		...
		for (step = 1; step < max_flips; ++step)
		{
			...
			if (hard_unsat_nb == 0 && (soft_unsat_weight < opt_unsat_weight || best_soln_feasible == 0))
			{
				...
				if (soft_unsat_weight < top_clause_weight)
				{
					...
					//  printf("opt_unsat_weight = %lld, opt_time = %.2f\n",opt_unsat_weight,opt_time);
			...
			if (step % 1000 == 0)
			{
				...
				if (mark == 0 && elapse_time > 300) // result 300s
				{
					if (best_soln_feasible == 1)
					{
				...
				else if (elapse_time >= cutoff_time / 2)
				{
					if (best_soln_feasible == 1)
					{
						opt_unsat_weight_vo = opt_unsat_weight;
						opt_time_vo = opt_time;
						suc_vo = 1;
						for (int v = 1; v <= num_vars; ++v)
						{
							best_soln_vo[v] = best_soln[v];
						}
					}
					else
					{
						opt_unsat_weight_vo = 9223372036;
						opt_time_vo = -1;
					}
					return;
				}
			...
			if ((step_count % (100000 * xishu)) == (100000 * xishu - 1)) // һ�� ʮ��
			{
				// printf("----turb begin------------------------------\n");
...
void Satlike::local_search_vt(vector<int> &init_solution, char *inputfile)
{
	int step_count = 0;
	int cur_hard_unsat_nb = 0;
	int xishu;
	...
	for (tries = 1; tries < max_tries; ++tries)
	{
		max_flips = 10000;
		mark_tries++;
		init_vt(init_solution);
		cur_hard_unsat_nb = hard_unsat_nb;
		int init_hard_unsat = hard_unsat_nb;
		for (step = 1; step < max_flips; ++step)
		{
			step_count++;
			if (hard_unsat_nb < init_hard_unsat)
			{
				step = step / 2 + 1;
				init_hard_unsat = hard_unsat_nb;
			}
			...
			if ((step_count % (100000 * xishu)) == (100000 * xishu - 1))
			{
				// printf("----turb begin------------------------------\n");
				// printf("hard_unsat_nb = %d, run time =%.2f \n",hard_unsat_nb,get_runtime());
				if (hard_unsat_nb <= 10 && rand() % 100 < 50)
...
void Satlike::turb()
{
		...
		for (c = 0; c < 10; c++)
		{
			...
			for (lit *p = clause_lit[sel_c]; (v = p->var_num) != 0; p++)
			{
				if (var_mark[v] == 0)
				{
					...
					if (hard_unsat_nb <= 50 && ss == 1 && rand() % 100 < 50 && cur_soln[v] == 1) // && ((ss == 1 && cur_soln[v] == 1) || (ss == 2 && cur_soln[v] == 1)))
					{
						turb_flipvar = v;
						flip(turb_flipvar);
						step_sum++;
	...
	/*turb*/
	// printf("count_var_mark =%d, theshold = %d, hard_unsat_nb=%d\n",count_var_mark,theshold,hard_unsat_nb);
	int turb_step;
	...
	int ttt = 0;
	int turb_best_var;
	for (turb_step = 1; turb_step <= 50; turb_step++)
	{
		ttt++;
...
int Satlike::turb_pick_var(int last_flip_var)
{
	...
	int turb_size = 1;
	int pos_size = 1;
	long long min_pick_score = LLONG_MAX;
	int turb_seed;
...
void Satlike::get_obj(string opb_file_name)
{
	...
	if (num_vars == num_vars_opb)
	{
		for (i = 1; i <= num_vars; i++)
		{
			obj_vo += best_soln_vo[i] * opb[i];
			obj_3600 += best_soln[i] * opb[i];
		}
		if (suc_vo == 0 && suc_vt == 0)
		{
			best_soln_feasible = 0;
		}
		else if (suc_vo == 1 && suc_vt == 0)
		{
			for (i = 1; i <= num_vars; i++)
			{
				best_soln[i] = best_soln_vo[i];
			}
			obj_3600 = obj_vo;
			opt_time = opt_time_vo;
			opt_unsat_weight = opt_unsat_weight_vo;
		}
		else if (suc_vo == 1 && suc_vt == 1)
		{
			if (obj_vo < obj_3600)
			{
				for (i = 1; i <= num_vars; i++)
